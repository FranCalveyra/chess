...
public class MoveNotIntoCheck implements PreMovementRule{
    ...
    public boolean isValidRule(ChessMove move, ChessGame game) {
        ...
        for (ChessMove moveToDo : playToExecute) {
            resultPair = game.getMoveExecutor().executeMove(moveToDo.from(), moveToDo.to(), resultPair.first(), game.getPromoter());
        }
        Pair<Board, ChessMoveResult> finalResultPair = resultPair;
        return checks.stream().anyMatch(check -> check.team() == game.getBoard().pieceAt(move.from()).getPieceColour() &&  !check.isValidRule(finalResultPair.first()));
    }
}
