...
public class PawnFirstMove implements PieceMovement {
  ...
  public boolean isValidMove(Position oldPos, Position newPos, Board context) {
    ...
    boolean originalPos =
        piece.getPieceColour() == Color.BLACK ? oldPos.getRow() == context.getRows()-2 : oldPos.getRow() == 1;
    return horizontalMove && verticalMove && originalPos && isNotPieceBetween(oldPos, newPos, context);
  }
}
