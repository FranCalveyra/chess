...
public class TreePreMovementValidator implements  PreMovementValidator{
    ...
    public TreePreMovementValidator(PreMovementRule rule, TreePreMovementValidator leftValidator, TreePreMovementValidator rightValidator) {
        this.rule = rule;
    ...
    private boolean getValidity(ChessMove move, ChessGame game) {
        if (rule == null){
            ...
            return left.getMoveValidity(move, game) == VALID_MOVE && right.getMoveValidity(move, game) == VALID_MOVE;
        }
        if(left == null && right != null){
        ...
        return left.getMoveValidity(move, game) == VALID_MOVE && right.getMoveValidity(move, game) == VALID_MOVE && rule.isValidRule(move,game);
    }
