...
public class CheckersTest {
  ...
  private CheckersPieceProvider pieceProvider = new CheckersPieceProvider();

  @Test
  public void initialMovement() {
    GameResult result =
        game.makeMove(
            new GameMove(fromAlgebraic("a3"), fromAlgebraic("b4")));
    assertEquals(new ValidPlay(), result.moveResult());
  }
  @Test
  public void promotionShouldWork(){
    GameResult result;
    Board board = new MapBoard(Map.of(fromAlgebraic("a7"), pieceProvider.provideCheckersPiece(Color.RED, CheckersType.MAN)));
    game = new ChessGame(board, game.getWinConditions(), game.getCheckConditions(), game.getPromoter(), game.getTurnSelector(), game.getPreMovementValidator());
    result = game.makeMove(new GameMove(fromAlgebraic("a7"), fromAlgebraic("b8")));
    assertEquals(new GameWon(Color.RED),result.moveResult());
    assertEquals(CheckersType.KING, ((ChessGame) result.game()).getBoard().pieceAt(fromAlgebraic("b8")).getType());
  }
}
